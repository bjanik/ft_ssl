...
static void	set_bn_to_data(t_bn *n, unsigned char *data, uint32_t *len)
{
	...
	data[(*len)++] = 0x02;
	set_len_to_data(bn_get_byte_number(n), data, len);
	bytes = bn_get_byte_number_last_limb(n->num[SIZE(n) - 1]);
...
static void	fill_pem_data(t_rsa_data *rsa, unsigned char *data, uint32_t data_len)
{
	...
	data[len++] = 0x30;
	data[len++] = 0x82;
	set_len_to_data(data_len - 4, data, &len);
...
{
	uint32_t 		data_len, data_encoded_len, offset;
	unsigned char 	*data, *data_encoded;
