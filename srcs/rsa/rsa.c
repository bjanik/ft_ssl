...
static int 			private_routine(t_rsa *rsa, char *args[],
						 unsigned char **raw_data, uint32_t *raw_data_len)
{
	int ret;
	...
	{
		args[1] = rsa->passin;
		if ((*raw_data = private_key_decryption(rsa->des, *raw_data, raw_data_len, args)) == NULL)
			return (1);
	}
	...
	if ((rsa->opts & RSA_DES) && (rsa->opts & RSA_PUBOUT) == 0)
	{
		...
		args[1] = rsa->passout;
		if ((*raw_data = private_key_encryption(rsa->des, *raw_data, raw_data_len, args)) == NULL)
			return (1);
	}
	return (0);
}
