...
void	my_putnbr(int nbr, char *base, int size_base, int fd)
{
	long nb;
	...
	{
		my_putnbr(nb / size_base, base, size_base, fd);
		ft_putchar_fd(base[nb % size_base], fd);
	}
}
...
void	ft_putnbr_base_fd(int nbr, char *base, int fd)
{
	...
	if (check_base(base) == 0)
		...
		my_putnbr(nbr, base, size_base, fd);
}
